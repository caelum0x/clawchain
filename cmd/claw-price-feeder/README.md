# ClawChain Oracle Price Feeder

The `claw-price-feeder` tool is an extension of ClawChain's `x/oracle` module.
It is forked from [Ojo Network's price-feeder](https://github.com/ojo-network/price-feeder),
which is based on Terra's [x/oracle](https://github.com/terra-money/classic-core/tree/main/x/oracle)
...
- The `claw-price-feeder` combines both `feeder` and `price-server` into a single
  Golang-based application for better UX, testability, and integration.

## Background

The `claw-price-feeder` tool is responsible for performing the following:
...
2. Taking aggregated exchange rate price data and submitting those exchange rates
   on-chain to ClawChain's `x/oracle` module following ClawChain's Oracle
   specification.
...
The `claw-price-feeder` tool runs off of one or many configuration files.
...
The files in the provider-config folder define what exchange rates to fetch and what providers to get them from. They also contain deviation thresholds and API endpoints. Please see the [example configuration](clawchain-provider-config) for more details.

```shell
$ claw-price-feeder /path/to/price_feeder_config.toml
```
...
`claw-price-feeder` process such the listening address and various HTTP timeouts.
...
The `rpc` section contains the CometBFT and Cosmos application gRPC endpoints.
