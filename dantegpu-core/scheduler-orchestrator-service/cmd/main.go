...
func main() {
	...
	billingConfig := &billing.Config{
		BaseURL: cfg.BillingServiceBaseURL,
		Timeout: cfg.BillingServiceTimeout,
	}
	billingClient := billing.NewClient(billingConfig, logger)
	logger.Info("Billing Service client initialized",
		zap.String("base_url", cfg.BillingServiceBaseURL),
		zap.Duration("timeout", cfg.BillingServiceTimeout),
	)
