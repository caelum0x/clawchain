...
func (jc *JobConsumer) StartConsuming() error {
	...
	if err := jc.startStatusUpdatesSubscription(); err != nil {
		return err
	}
	if err := jc.startCancelSubscription(); err != nil {
		return err
	}
...
func (jc *JobConsumer) startStatusUpdatesSubscription() error {
	subject := fmt.Sprintf("%s.*", jc.cfg.NatsJobStatusUpdateSubjectPrefix)
	sub, err := jc.nc.Subscribe(subject, jc.handleStatusUpdateMessage)
	if err != nil {
		jc.logger.Error("Failed to subscribe to job status updates", zap.String("subject", subject), zap.Error(err))
		return fmt.Errorf("subscribe status updates: %w", err)
	}
	jc.statusSub = sub
	jc.logger.Info("Subscribed to provider status updates", zap.String("subject", subject))
	return nil
}

func (jc *JobConsumer) startCancelSubscription() error {
	sub, err := jc.nc.Subscribe(jc.cfg.NatsJobCancelSubject, jc.handleCancelMessage)
	if err != nil {
		jc.logger.Error("Failed to subscribe to job cancel requests", zap.String("subject", jc.cfg.NatsJobCancelSubject), zap.Error(err))
		return fmt.Errorf("subscribe cancel requests: %w", err)
	}
	jc.cancelSub = sub
	jc.logger.Info("Subscribed to job cancel requests", zap.String("subject", jc.cfg.NatsJobCancelSubject))
	return nil
}
...
func (jc *JobConsumer) scheduleJob(internalJob *models.InternalJobRepresentation) (bool, error) {
	...
	for _, p := range providers {
		...
		// VRAM matching: if job specifies minimum VRAM, verify provider has a GPU with enough
		if job.MinVRAMMB > 0 {
			hasEnoughVRAM := false
		...
		// GPU model matching: if job specifies a model name, check provider has it
		if job.GPUModel != "" {
			hasModel := false
			for _, gpu := range provider.GPUs {
	...
	if jc.billingClient != nil {
		...
		if err != nil {
			// Keep dispatching even if billing is temporarily unavailable.
			jc.logger.Warn("Job billing validation failed; continuing without billing gate",
				zap.String("job_id", job.ID),
				zap.Error(err),
			)
		}
		...
		if err != nil {
			jc.logger.Warn("Failed to start billing session; continuing job dispatch",
				zap.String("job_id", job.ID),
				zap.Error(err),
			)
		} else {
	...
	dispatchProviderID := selectDispatchProviderID(suitableProvider)
	task := models.NewTask(&job, dispatchProviderID)
	...
	dispatchSubject := fmt.Sprintf("%s.%s.%s", jc.cfg.NatsTaskDispatchSubjectPrefix, dispatchProviderID, job.ID)
	jc.logger.Info("Task created, attempting to dispatch to NATS",
		zap.String("job_id", job.ID),
		zap.String("provider_id", dispatchProviderID),
	...
	internalJob.ProviderID = dispatchProviderID
...
func (jc *JobConsumer) handleStatusUpdateMessage(msg *nats.Msg) {
	var update taskStatusUpdate
	if err := json.Unmarshal(msg.Data, &update); err != nil {
		jc.logger.Warn("Invalid provider status update payload", zap.Error(err), zap.String("subject", msg.Subject))
		return
	}
	if update.JobID == "" {
		jc.logger.Warn("Ignoring provider status update without job_id", zap.String("subject", msg.Subject))
		return
	}

	newState := mapProviderStatusToSchedulerState(update.Status)
	if newState == "" {
		jc.logger.Debug("Ignoring unmapped provider status", zap.String("job_id", update.JobID), zap.String("status", update.Status))
		return
	}
	...
	record, err := jc.jobStore.GetJob(ctx, update.JobID)
	if err != nil {
		jc.logger.Error("Failed to read job before status update", zap.String("job_id", update.JobID), zap.Error(err))
		return
	}
	if record == nil {
		jc.logger.Warn("Ignoring provider status for unknown job", zap.String("job_id", update.JobID))
		return
	}
	...
	if err := jc.jobStore.UpdateJobState(ctx, update.JobID, newState, providerID, update.Message, record.Attempts); err != nil {
		jc.logger.Error("Failed to persist provider status update",
	...
	jc.logger.Info("Persisted provider status update",
		zap.String("job_id", update.JobID),
...
func (jc *JobConsumer) handleCancelMessage(msg *nats.Msg) {
	var req jobCancelRequest
	if err := json.Unmarshal(msg.Data, &req); err != nil {
		jc.logger.Warn("Invalid cancel payload", zap.Error(err), zap.String("subject", msg.Subject))
		return
	}
	if req.JobID == "" {
		jc.logger.Warn("Ignoring cancel payload without job_id")
		return
	}
	...
	record, err := jc.jobStore.GetJob(ctx, req.JobID)
	if err != nil {
		jc.logger.Error("Failed to load job for cancellation", zap.String("job_id", req.JobID), zap.Error(err))
		return
	}
	if record == nil {
		jc.logger.Warn("Cancel requested for unknown job", zap.String("job_id", req.JobID))
		return
	}
	if record.State == models.JobStateCompleted || record.State == models.JobStateFailed || record.State == models.JobStateCancelled {
		jc.logger.Info("Cancel ignored: job already terminal", zap.String("job_id", req.JobID), zap.String("state", string(record.State)))
		return
	}

	providerID := record.ProviderID
	if providerID != "" {
		cancelSubject := fmt.Sprintf("%s.%s.%s", jc.cfg.NatsTaskCancelSubjectPrefix, providerID, req.JobID)
		payload, _ := json.Marshal(req)
		if err := jc.nc.Publish(cancelSubject, payload); err != nil {
			jc.logger.Error("Failed to publish task cancel command",
				zap.String("job_id", req.JobID),
				zap.String("subject", cancelSubject),
				zap.Error(err),
			)
			return
		}
		jc.logger.Info("Published task cancel command",
			zap.String("job_id", req.JobID),
			zap.String("subject", cancelSubject),
		)
	}

	if err := jc.jobStore.UpdateJobState(ctx, req.JobID, models.JobStateCancelled, providerID, "cancel requested", record.Attempts); err != nil {
		jc.logger.Error("Failed to persist cancelled state", zap.String("job_id", req.JobID), zap.Error(err))
		return
	}
	jc.logger.Info("Cancel processed", zap.String("job_id", req.JobID), zap.String("provider_id", providerID))
}

func mapProviderStatusToSchedulerState(status string) models.SchedulerJobState {
	switch strings.ToLower(status) {
	case "preparing":
		return models.JobStateDispatched
	case "in_progress":
		return models.JobStateRunning
	case "completed":
		return models.JobStateCompleted
	case "cancelled":
		return models.JobStateCancelled
	case "failed", "timeout":
		return models.JobStateFailed
	default:
		return ""
	}
}

func selectDispatchProviderID(provider *clients.Provider) string {
	// Use provider name when available, because provider-daemon uses InstanceID routing.
	if trimmed := strings.TrimSpace(provider.Name); trimmed != "" {
		return sanitizeSubjectToken(trimmed)
	}
	return sanitizeSubjectToken(provider.ID.String())
}
...
func (jc *JobConsumer) Stop() {
	...
	if jc.statusSub != nil {
		_ = jc.statusSub.Unsubscribe()
	}
	if jc.cancelSub != nil {
		_ = jc.cancelSub.Unsubscribe()
	}
