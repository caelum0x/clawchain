...
type Config struct {
	...
	NatsJobCancelSubject             string `yaml:"nats_job_cancel_subject"`
	...
	NatsTaskCancelSubjectPrefix      string `yaml:"nats_task_cancel_subject_prefix"`
	...
	BillingServiceBaseURL string        `yaml:"billing_service_base_url"`
	BillingServiceTimeout time.Duration `yaml:"billing_service_timeout"`
}
...
func LoadConfig(path string) (*Config, error) {
	defaultConfig := &Config{
		...
		BillingServiceBaseURL: "http://localhost:8082",
		BillingServiceTimeout: 30 * time.Second,
	}
...
func applyDefaultsIfNotSet(cfg *Config, defaults *Config) {
	...
	if cfg.NatsJobCancelSubject == "" {
		cfg.NatsJobCancelSubject = defaults.NatsJobCancelSubject
	}
	...
	if cfg.NatsTaskCancelSubjectPrefix == "" {
		cfg.NatsTaskCancelSubjectPrefix = defaults.NatsTaskCancelSubjectPrefix
	}
	...
	if cfg.BillingServiceBaseURL == "" {
		cfg.BillingServiceBaseURL = defaults.BillingServiceBaseURL
	}
	if cfg.BillingServiceTimeout == 0 {
		cfg.BillingServiceTimeout = defaults.BillingServiceTimeout
	}
}
...
func applyEnvOverrides(cfg *Config) {
	if v := os.Getenv("BILLING_SERVICE_URL"); v != "" {
		cfg.BillingServiceBaseURL = v
	}
	if v := os.Getenv("BILLING_SERVICE_TIMEOUT"); v != "" {
		if seconds, err := strconv.Atoi(v); err == nil && seconds > 0 {
			cfg.BillingServiceTimeout = time.Duration(seconds) * time.Second
		}
	}
}
