...
func main() {
	...
	// Subscribe to provider-daemon task status updates.
	jobStatusSubject := fmt.Sprintf("%s.*", cfg.NatsJobStatusUpdateSubjectPrefix)
	if _, err := nc.Subscribe(jobStatusSubject, jobHandler.HandleTaskStatusMessage); err != nil {
		logger.Fatal("Failed to subscribe to task status updates",
			zap.String("subject", jobStatusSubject),
			zap.Error(err),
		)
	}
	logger.Info("Subscribed to provider task status updates", zap.String("subject", jobStatusSubject))
	...
	r.Route("/api/v1", func(r chi.Router) {
		// Provider-facing task lifecycle endpoints (Dante-compatible).
		r.Post("/tasks", jobHandler.SubmitTask)
		r.Get("/tasks/{taskID}/status", jobHandler.GetTaskStatus)
		r.Post("/tasks/{taskID}/cancel", jobHandler.CancelTask)
