...
func RegisterService(consulClient *consulapi.Client, cfg *config.Config, serviceID string, logger *zap.Logger) error {
	...
	// In Docker, empty address causes checks to target Consul container localhost.
	// Use logical service name as routable address when host is unspecified.
	address := host
	if address == "" {
		address = cfg.ServiceName
	}
