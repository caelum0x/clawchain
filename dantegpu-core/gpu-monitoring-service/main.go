...
func (gms *GPUMonitoringService) handleCreateAllocation(w http.ResponseWriter, r *http.Request) {
	...
	if strings.TrimSpace(req.GPUID) == "" || strings.TrimSpace(req.JobID) == "" || strings.TrimSpace(req.UserID) == "" {
		http.Error(w, "gpu_id, job_id, and user_id are required", http.StatusBadRequest)
		return
	}

	gms.dataMu.Lock()
	gpu, exists := gms.gpuData[req.GPUID]
	if !exists {
		gms.dataMu.Unlock()
		http.Error(w, "gpu not found", http.StatusNotFound)
		return
	}
	if req.AllocatedAt.IsZero() {
		req.AllocatedAt = time.Now()
	}
	if req.EstimatedEnd.IsZero() {
		req.EstimatedEnd = req.AllocatedAt.Add(1 * time.Hour)
	}
	if req.Status == "" {
		req.Status = "active"
	}
	...
	for _, alloc := range gpu.Allocations {
		if alloc.JobID == req.JobID {
			gms.dataMu.Unlock()
			http.Error(w, "allocation already exists for job_id", http.StatusConflict)
			return
		}
	}

	gpu.Allocations = append(gpu.Allocations, req.GPUAllocation)
	gpu.IsAvailable = gpu.UtilizationGPU < 80 && gpu.MemoryUsed < (gpu.MemoryTotal*80/100)
	gms.dataMu.Unlock()
...
func (gms *GPUMonitoringService) handleDeleteAllocation(w http.ResponseWriter, r *http.Request) {
	...
	deleted := 0
	for _, gpu := range gms.gpuData {
		next := gpu.Allocations[:0]
		for _, alloc := range gpu.Allocations {
			if alloc.JobID == allocationID {
				deleted++
				continue
			}
			next = append(next, alloc)
		}
		gpu.Allocations = next
	}
	gms.dataMu.Unlock()
	...
	gms.logger.Info("Deleted allocation", zap.String("id", allocationID), zap.Int("records_deleted", deleted))
	...
	for _, gpu := range gms.gpuData {
		...
		for _, alloc := range gpu.Allocations {
			if alloc.Status == "active" {
				...
				activeJobs++
			} else if alloc.Status == "queued" {
				queuedJobs++
			}
			userAllocations[alloc.UserID] = map[string]interface{}{
				"job_id":        alloc.JobID,
				"status":        alloc.Status,
				"gpu_fraction":  alloc.GPUFraction,
				"memory_slice":  alloc.MemorySlice,
				"estimated_end": alloc.EstimatedEnd,
			}
		}
	}
