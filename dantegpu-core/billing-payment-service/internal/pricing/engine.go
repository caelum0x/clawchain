...
type Config struct {
	// Base rates by GPU model (CLAW tokens per hour)
	BaseRates map[string]float64 `yaml:"base_rates"`

	// VRAM pricing (CLAW tokens per GB per hour)
...
// Uses request parameters to apply simple heuristics: premium GPUs (high VRAM) get a
// demand multiplier, and long-duration jobs get a supply discount bonus.
func (e *Engine) getDynamicPricingFactors(ctx context.Context, req *PricingRequest) (demandMultiplier, supplyBonus decimal.Decimal, err error) {
	...
	// Premium GPUs (>= 24 GB VRAM) have higher demand
	if req.TotalVRAM >= 24*1024 { // 24 GB in MB
		demandMultiplier = decimal.NewFromFloat(1.15)
	}

	// Long-duration jobs (>= 24 hours) get a supply bonus (volume discount)
	if req.DurationHours.GreaterThanOrEqual(decimal.NewFromInt(24)) {
		supplyBonus = decimal.NewFromFloat(0.05)
	}
...
// GetSupportedGPUModels returns supported GPU models and their base hourly rates.
...
// GetSupporteCLAWModels is kept as a temporary compatibility alias for older callers.
func (e *Engine) GetSupporteCLAWModels() map[string]decimal.Decimal {
	return e.GetSupportedGPUModels()
}
