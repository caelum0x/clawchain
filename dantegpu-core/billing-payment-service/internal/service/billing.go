...
import (
	...
	"github.com/dante-gpu/dante-backend/billing-payment-service/internal/clawchain"
...
type BillingService struct {
	...
	chainClient   *clawchain.ClawChainClient
...
type Config struct {
	...
	AutoProvisionUserWallets    bool            `yaml:"auto_provision_user_wallets"`
	AutoProvisionInitialBalance decimal.Decimal `yaml:"auto_provision_initial_balance"`
}
...
// CreateWallet creates a new CLAW token wallet for a user or provider.
// On ClawChain, accounts are implicit — we only validate the bech32 address.
func (s *BillingService) CreateWallet(ctx context.Context, req *models.WalletCreateRequest) (*models.Wallet, error) {
	s.logger.Info("Creating wallet",
		...
		zap.String("clawchain_address", req.ClawChainAddress),
	)

	// Validate ClawChain bech32 address format
	if !isValidClawChainAddress(req.ClawChainAddress) {
		return nil, models.NewValidationError("clawchain_address", "invalid ClawChain address format (expected claw1...)")
	}
...
func (s *BillingService) GetWalletBalance(ctx context.Context, walletID uuid.UUID) (*models.BalanceResponse, error) {
	...
	chainBalance, err := s.chainClient.GetBalance(ctx, wallet.ClawChainAddress)
	if err != nil {
		s.logger.Warn("Failed to get chain balance, using database balance", zap.Error(err))
		chainBalance = wallet.Balance
	}

	// Update database balance if there's a significant difference
	if chainBalance.Sub(wallet.Balance).Abs().GreaterThan(decimal.NewFromFloat(0.001)) {
		err = s.store.UpdateWalletBalance(ctx, walletID, chainBalance, wallet.LockedBalance)
		if err != nil {
...
func (s *BillingService) StartRentalSession(ctx context.Context, req *models.SessionStartRequest) (*models.SessionResponse, error) {
	...
	// Get or auto-provision user wallet for first-time users.
	userWallet, err := s.getOrProvisionUserWallet(ctx, req.UserID)
	if err != nil {
		return nil, err
	}
...
func (s *BillingService) getOrProvisionUserWallet(ctx context.Context, userID string) (*models.Wallet, error) {
	wallet, err := s.store.GetWalletByUserID(ctx, userID, models.WalletTypeUser)
	if err == nil {
		return wallet, nil
	}
	if err != models.ErrWalletNotFound || !s.config.AutoProvisionUserWallets {
		return nil, err
	}

	wallet, err = s.store.CreateWallet(ctx, &models.WalletCreateRequest{
		UserID:           userID,
		WalletType:       models.WalletTypeUser,
		ClawChainAddress: syntheticUserAddress(userID),
	})
	if err != nil {
		// Handle race where another request provisioned in parallel.
		existing, getErr := s.store.GetWalletByUserID(ctx, userID, models.WalletTypeUser)
		if getErr == nil {
			return existing, nil
		}
		return nil, err
	...
	initialBalance := s.config.AutoProvisionInitialBalance
	if initialBalance.LessThan(s.config.MinimumBalance) {
		initialBalance = s.config.MinimumBalance
	}
	if initialBalance.GreaterThan(decimal.Zero) {
		wallet.Balance = initialBalance
		if updateErr := s.store.UpdateWalletBalance(ctx, wallet.ID, wallet.Balance, wallet.LockedBalance); updateErr != nil {
			return nil, updateErr
		}
	}

	s.logger.Info("Auto-provisioned user wallet",
		zap.String("user_id", userID),
		zap.String("wallet_id", wallet.ID.String()),
		zap.String("initial_balance", initialBalance.String()),
	)

	return wallet, nil
}

func syntheticUserAddress(userID string) string {
	sum := sha256.Sum256([]byte("billing-user:" + userID))
	// 38 chars after prefix keeps total length in expected range.
	return "claw1" + hex.EncodeToString(sum[:])[:38]
}
...
// ProcessDeposit processes a CLAW token deposit by confirming the on-chain tx.
func (s *BillingService) ProcessDeposit(ctx context.Context, req *models.DepositRequest) (*models.Transaction, error) {
	s.logger.Info("Processing deposit",
		...
		zap.String("tx_hash", req.ChainTxHash),
	)

	// Verify the ClawChain transaction
	err := s.chainClient.ConfirmTransaction(ctx, req.ChainTxHash)
	if err != nil {
		return nil, models.NewChainError("confirm_deposit", err)
	}
	...
	txnReq := &models.TransactionCreateRequest{
		...
		Amount:      req.Amount,
		Description: "CLAW token deposit",
		Metadata: map[string]interface{}{
			"chain_tx_hash": req.ChainTxHash,
		},
	}
	...
	err = s.store.UpdateTransactionStatus(ctx, transaction.ID, models.TransactionStatusConfirmed, &req.ChainTxHash)
	if err != nil {
...
// ProcessWithdrawal processes a CLAW token withdrawal via on-chain MsgSend.
func (s *BillingService) ProcessWithdrawal(ctx context.Context, req *models.WithdrawalRequest) (*models.Transaction, error) {
	...
	txnReq := &models.TransactionCreateRequest{
		...
		Amount:       req.Amount,
		Description:  fmt.Sprintf("CLAW token withdrawal to %s", req.ToAddress),
		Metadata: map[string]interface{}{
	...
	// Execute ClawChain transfer
	txHash, err := s.chainClient.TransferTokens(ctx, wallet.ClawChainAddress, req.ToAddress, req.Amount)
	if err != nil {
		// Update transaction as failed
		s.store.UpdateTransactionStatus(ctx, transaction.ID, models.TransactionStatusFailed, nil)
		return nil, models.NewChainError("transfer_tokens", err)
	}
	...
	// Update transaction with confirmed status and tx hash
	err = s.store.UpdateTransactionStatus(ctx, transaction.ID, models.TransactionStatusConfirmed, &txHash)
	if err != nil {
		s.logger.Warn("Failed to update transaction status", zap.Error(err))
	}
...
// isValidClawChainAddress validates a ClawChain bech32 address format.
