...
func NewBillingHandler(db *sql.DB, chainClient *blockchain.ChainClient, platformFee float64) *BillingHandler {
...
func (h *BillingHandler) EndRental(ctx context.Context, sessionID, reason string) error {
	...
	_ = escrowAmount - totalCost // refund is implicit
	...
	// Process on-chain payout to provider (async)
	go h.processProviderPayout(context.Background(), providerID, providerWalletAddr, providerAmount)

	return nil
}

// processProviderPayout sends CLAW payment to provider via on-chain MsgSend.
func (h *BillingHandler) processProviderPayout(ctx context.Context, providerID, address string, amount float64) {
	if !blockchain.IsValidAddress(address) {
		...
		`, "failed", "invalid provider address", providerID)
		return
	}

	// Convert CLAW to uclaw
	amountUclaw := uint64(amount * 1e6)

	// Transfer from platform wallet to provider on ClawChain
	result, err := h.chainClient.TransferTokens(ctx, address, amountUclaw)
	if err != nil {
		h.db.ExecContext(ctx, `
			UPDATE provider_payouts
			SET status = $1, error = $2
	...
	h.db.ExecContext(ctx, `
		UPDATE provider_payouts
		SET status = $1, tx_hash = $2, paid_at = $3
		WHERE provider_id = $4 AND status = 'pending'
	`, "completed", result.TxHash, time.Now(), providerID)
}
