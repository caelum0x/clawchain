...
type Handler struct {
	...
	activeCancels  sync.Map // Stores context.CancelFunc, keyed by JobID
	terminalSent   sync.Map // Stores bool keyed by JobID to avoid duplicate terminal status publish
}
...
func (h *Handler) runTask(task *models.Task) {
	...
	h.activeCancels.Store(task.JobID, cancelFunc)
	defer func() {
		h.activeCancels.Delete(task.JobID)
		cancelFunc()
	}()

	ctx, timeoutCancel := context.WithTimeout(baseCtx, h.cfg.RequestTimeout)
	defer timeoutCancel()

	if task.ExecutionType == models.ExecutionTypeUndefined {
		task.ExecutionType = resolveExecutionType(task)
	}
	...
	if errors.Is(ctx.Err(), context.Canceled) {
		...
		finalMessage = "Task cancelled by request"
		executionLog = fmt.Sprintf("Stdout:\n%s\nStderr:\n%s", result.Stdout, result.Stderr)
	} else if result.Error != nil { // Error in execution setup or process
...
func (h *Handler) reportTaskStatus(jobID string, status models.JobStatus, message string, exitCode *int, execLog string) error {
	...
	isTerminal := status == models.StatusFailed || status == models.StatusCompleted || status == models.StatusCancelled
	if isTerminal {
		if _, loaded := h.terminalSent.LoadOrStore(jobID, true); loaded {
			h.logger.Info("Skipping duplicate terminal status publish", zap.String("jobID", jobID), zap.String("status", string(status)))
			return nil
		}
	}
...
func (h *Handler) CancelTask(jobID string) error {
	cancelRaw, ok := h.activeCancels.Load(jobID)
	if !ok {
	...
	if !ok {
		return fmt.Errorf("cancel function not found for job %s", jobID)
	}
	cancel()

	_ = h.reportTaskStatus(jobID, models.StatusCancelled, "Task cancellation requested", nil, "")
	return nil
}

func resolveExecutionType(task *models.Task) models.ExecutionType {
	if _, ok := task.JobParams["docker_image"]; ok {
		return models.ExecutionTypeDocker
	}
	return models.ExecutionTypeScript
}
