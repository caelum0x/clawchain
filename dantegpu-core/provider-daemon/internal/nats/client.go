...
type TaskCancelFunc func(jobID string) error
...
type Client struct {
	...
	cancelHandler TaskCancelFunc
	shutdownChan  chan struct{}
	js            nats.JetStreamContext // JetStream context
}

// NewClient creates a new NATS client for the provider daemon.
func NewClient(cfg *config.Config, logger *zap.Logger, handler TaskHandlerFunc, cancelHandler TaskCancelFunc) (*Client, error) {
	client := &Client{
		logger:        logger,
		cfg:           cfg,
		taskHandler:   handler,
	...
	nc, err := nats.Connect(
		...
		nats.ReconnectHandler(func(nc *nats.Conn) {
			...
			if err := client.startCancelSubscription(); err != nil {
				logger.Error("Failed to restart cancel subscription after NATS reconnect", zap.Error(err))
			}
		}),
...
func (c *Client) StartListening() error {
	...
	return c.startCancelSubscription()
}
...
func (c *Client) startCancelSubscription() error {
	if c.nc == nil {
		return fmt.Errorf("NATS connection not available for cancel subscription")
	}
	if c.cancelSub != nil && c.cancelSub.IsValid() {
		_ = c.cancelSub.Unsubscribe()
	}
	subject := fmt.Sprintf(c.cfg.NatsConfig.TaskCancelSubjectPattern, c.cfg.InstanceID)
	sub, err := c.nc.Subscribe(subject, c.handleCancelMessage)
	if err != nil {
		c.logger.Error("Failed to subscribe to cancel subject", zap.String("subject", subject), zap.Error(err))
		return fmt.Errorf("subscribe cancel subject: %w", err)
	}
	c.cancelSub = sub
	c.logger.Info("Subscribed to task cancellation subject", zap.String("subject", subject))
	return nil
}
...
func (c *Client) handleCancelMessage(msg *nats.Msg) {
	if c.cancelHandler == nil {
		c.logger.Warn("Cancel handler not set; ignoring cancel message", zap.String("subject", msg.Subject))
		return
	}
	...
	if err := json.Unmarshal(msg.Data, &payload); err != nil {
		c.logger.Warn("Invalid cancel message payload", zap.Error(err), zap.String("subject", msg.Subject))
		return
	}
	if payload.JobID == "" {
		tokens := strings.Split(msg.Subject, ".")
		if len(tokens) > 0 {
			payload.JobID = tokens[len(tokens)-1]
		}
	}
	if payload.JobID == "" {
		c.logger.Warn("Cancel message missing job_id", zap.String("subject", msg.Subject))
		return
	}
	if err := c.cancelHandler(payload.JobID); err != nil {
		c.logger.Error("Failed to cancel active task", zap.String("job_id", payload.JobID), zap.Error(err))
		return
	}
	c.logger.Info("Cancel command handled", zap.String("job_id", payload.JobID))
}
...
func (c *Client) Stop() {
	...
	if c.cancelSub != nil && c.cancelSub.IsValid() {
		if err := c.cancelSub.Unsubscribe(); err != nil {
			c.logger.Error("Error unsubscribing cancel subscription", zap.Error(err))
		}
	}
