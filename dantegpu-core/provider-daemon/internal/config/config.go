...
type NatsConfig struct {
	...
	TaskCancelSubjectPattern     string        `yaml:"task_cancel_subject_pattern"`
...
type Config struct {
	...
	DefaultHourlyRateCLAW float64 `yaml:"default_hourly_rate_claw"`
...
type GpuRentalConfigEntry struct {
	...
	CurrentHourlyRateCLAW float32 `yaml:"current_hourly_rate_claw"`
}
...
func LoadConfig(path string, logger *zap.Logger) (*Config, error) {
	...
	defaultConfig := &Config{
		...
		NatsConfig: NatsConfig{ // Initialize locally defined NatsConfig
			...
			TaskCancelSubjectPattern:     "tasks.cancel.%s.*",
			JobStatusUpdateSubjectPrefix: "jobs.status",
		...
		MaxConcurrentJobs:           1,
		PreferredCurrency:           "CLAW",
		DefaultHourlyRateCLAW:       1.0, // Default value
...
func applyDefaultsIfNotSet(cfg *Config, defaults *Config) {
	...
	if cfg.NatsConfig.TaskCancelSubjectPattern == "" {
		cfg.NatsConfig.TaskCancelSubjectPattern = defaults.NatsConfig.TaskCancelSubjectPattern
	}
	...
	if cfg.DefaultHourlyRateCLAW == 0 && defaults.DefaultHourlyRateCLAW != 0 { // Check for 0.0 for float
		cfg.DefaultHourlyRateCLAW = defaults.DefaultHourlyRateCLAW
	}
