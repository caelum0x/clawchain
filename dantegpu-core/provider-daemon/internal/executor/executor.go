...
func (de *DockerExecutor) Execute(ctx context.Context, task *models.Task, workspacePath string, logger *zap.Logger) ExecutionResult {
	...
	// Handle script_content: run inline to avoid host-path bind assumptions when daemon runs in a container.
	if scriptContent, scOK := task.JobParams["script_content"].(string); scOK && strings.TrimSpace(scriptContent) != "" {
		...
		cmdSlice = []string{interpreter, "-c", scriptContent}
		jobLogger.Info("Using inline script_content command for Docker execution",
	...
	// Stop billing only when a billing session was started in this executor.
	if de.billingClient != nil && billingStarted {
	...
	} else if de.billingClient != nil {
		jobLogger.Info("Billing was not started for this job; skipping StopBilling call.", zap.String("job_id", task.JobID))
	} else {
