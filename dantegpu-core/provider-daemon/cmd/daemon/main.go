...
func main() {
	...
	natsClient, err := nats.NewClient(cfg, logger, taskHandler.HandleTask, taskHandler.CancelTask)
	if err != nil {
...
func handleGetGpusJSON(cfg *config.Config, logger *zap.Logger) {
	...
	for _, systemGPU := range detectedSystemGPUs { // Iterate over gpu.GPUInfo
		...
		if rentalCfg, ok := gpuRentalConfigMap[systemGPU.ID]; ok {
			...
			if rentalCfg.IsAvailableForRent && rentalCfg.CurrentHourlyRateCLAW > 0 {
				rate := rentalCfg.CurrentHourlyRateCLAW // This is float32 in GpuRentalConfigEntry
				cliInfo.CurrentHourlyRateCLAW = &rate
			}
		}
...
func handleGetSettingsJSON(cfg *config.Config, logger *zap.Logger) {
	...
	settings := cli_models.CliProviderSettings{
		DefaultHourlyRateCLAW: float32(cfg.DefaultHourlyRateCLAW),
...
func handleUpdateSettingsJSON(cfg *config.Config, configFilePath string, logger *zap.Logger, settingsJSON string) {
	...
	cfg.DefaultHourlyRateCLAW = float64(newSettings.DefaultHourlyRateCLAW) // CliProviderSettings uses float32, config.Config uses float64
...
func handleSetGpuRentalConfig(cfg *config.Config, configFilePath string, logger *zap.Logger, gpuID string, newRate float64, newAvailabilityStr string) {
	...
	for i, rentalConfig := range cfg.GpuRentalConfigs {
		if rentalConfig.GpuID == gpuID {
			...
			if rateProvided {
				if cfg.GpuRentalConfigs[i].CurrentHourlyRateCLAW != float32(newRate) { // config stores float32
					cfg.GpuRentalConfigs[i].CurrentHourlyRateCLAW = float32(newRate)
					logger.Info("Updated GPU hourly rate", zap.String("gpu_id", gpuID), zap.Float32("rate_claw", float32(newRate)))
					updateMade = true
				}
			}
			if !updateMade {
	...
	if !found {
		...
		if rateProvided {
			newEntry.CurrentHourlyRateCLAW = float32(newRate)
			updateMade = true
		} else {
...
func handleGetLocalJobsJSON(cfg *config.Config, logger *zap.Logger /* taskHandler *tasks.Handler */) {
	...
	// Read job status from the file written periodically by the running daemon.
	statusFile := filepath.Join(cfg.WorkspaceDir, "job-status.json")
	data, err := os.ReadFile(statusFile)
	if err != nil {
		if os.IsNotExist(err) {
			logger.Debug("No job status file found; daemon may not be running or has no active jobs", zap.String("path", statusFile))
		} else {
			logger.Warn("Failed to read job status file", zap.String("path", statusFile), zap.Error(err))
		}
		outputJSON(localJobs, logger)
		return
	}

	if err := json.Unmarshal(data, &localJobs); err != nil {
		logger.Warn("Failed to parse job status file", zap.String("path", statusFile), zap.Error(err))
		localJobs = make([]cli_models.CliLocalJob, 0)
	}
...
func handleGetNetworkStatusJSON(cfg *config.Config, logger *zap.Logger) {
	...
	natsClient, err := nats.NewClient(cfg, logger, nil, nil) // Using cfg.NatsConfig is handled by NewClient
	if err != nil {
...
func handleGetFinancialSummaryJSON(cfg *config.Config, logger *zap.Logger) {
	...
	if err != nil {
		...
		summary.CurrentBalanceCLAW = financialDetails.CurrentBalanceCLAW
		summary.TotalEarnedCLAW = financialDetails.TotalEarnedCLAW
		summary.PendingPayoutCLAW = financialDetails.PendingPayoutCLAW
