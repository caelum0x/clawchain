...
type Task struct {
	...
	EstimatedCostCLAW decimal.Decimal `json:"estimated_cost_claw"`
}
...
type TaskStatusUpdate struct {
	...
	// Cost and billing
	ActualCostCLAW    decimal.Decimal `json:"actual_cost_claw"`
...
// ChainWalletManager manages ClawChain wallet operations for the provider.
...
type GPUProvider struct {
	...
	// Advanced components
	walletManager *ChainWalletManager
...
func getDefaultProviderConfig() *common.ProviderConfig {
	return &common.ProviderConfig{
		...
		ClawChainAddress:    os.Getenv("CLAWCHAIN_ADDRESS"),
...
func NewGPUProvider(config *common.ProviderConfig) (*GPUProvider, error) {
	...
	providerInstance := &common.Provider{
		...
		Metadata: common.ProviderMetadata{
			...
			ClawChainWallet:   config.ClawChainAddress,
			DockerEnabled:     config.EnableDocker,
		},
	}

	// Initialize ClawChain wallet manager
	walletManager, err := initializeChainWallet(config, logger)
	if err != nil {
		logger.Warn("Failed to initialize ClawChain wallet, continuing without it", zap.Error(err))
	}
...
// initializeChainWallet initializes the ClawChain wallet manager using HKDF key derivation.
func initializeChainWallet(config *common.ProviderConfig, logger *zap.Logger) (*ChainWalletManager, error) {
	if config.ClawChainAddress == "" {
		return nil, fmt.Errorf("ClawChain address not configured")
	}

	restURL := getenvDefault("CLAWCHAIN_REST_URL", "http://localhost:1317")
	chainID := getenvDefault("CLAWCHAIN_CHAIN_ID", "clawchain-1")
	denom := getenvDefault("CLAWCHAIN_DENOM", "uclaw")

	wm := &ChainWalletManager{
		address:    config.ClawChainAddress,
		restURL:    restURL,
		chainID:    chainID,
		denom:      denom,
	...
	// Derive signing key from mnemonic (env or file).
	mnemonic := os.Getenv("CLAWCHAIN_MNEMONIC")
	if mnemonic == "" {
		if path := os.Getenv("CLAWCHAIN_MNEMONIC_PATH"); path != "" {
	...
	if mnemonic != "" {
		...
		hkdfReader := hkdf.New(hash.New, seed, []byte("clawchain-provider"), []byte("secp256k1"))
		privBytes := make([]byte, 32)
		if _, err := io.ReadFull(hkdfReader, privBytes); err != nil {
			return nil, fmt.Errorf("key derivation failed: %w", err)
		}
		wm.privKeyHex = hex.EncodeToString(privBytes)
	} else {
		logger.Warn("No mnemonic configured — wallet is read-only (cannot sign transactions)")
	}
	...
	logger.Info("ClawChain wallet initialized successfully",
		zap.String("address", config.ClawChainAddress))
...
func (wm *ChainWalletManager) testConnection() error {
	...
	if resp.StatusCode >= 400 {
		return fmt.Errorf("balance query HTTP %d: %s", resp.StatusCode, string(body))
	}
	...
	wm.logger.Info("ClawChain wallet balance",
		zap.String("uclaw", result.Balance.Amount))

	return nil
}

// broadcastMsg signs and broadcasts a Cosmos SDK message. Returns tx hash.
func (wm *ChainWalletManager) broadcastMsg(ctx context.Context, msg map[string]interface{}) (string, error) {
	...
	privBytes, err := hex.DecodeString(wm.privKeyHex)
	if err != nil {
	...
	prefix := byte(0x02)
	if privKey.PublicKey.Y.Bit(0) == 1 {
		prefix = 0x03
	}
	pubKeyBytes := append([]byte{prefix}, padded...)
	...
	r, s, signErr := ecdsa.Sign(bytes.NewReader(signHash[:]), privKey, signHash[:])
	if signErr != nil {
	...
	url := fmt.Sprintf("%s/cosmos/tx/v1beta1/txs", wm.restURL)
	httpReq, err := http.NewRequestWithContext(ctx, "POST", url, bytes.NewReader(broadcastReq))
	if err != nil {
		return "", err
	}
...
func (wm *ChainWalletManager) ensureInitialized(ctx context.Context) error {
	...
	body, _ := io.ReadAll(resp.Body)
	if resp.StatusCode >= 400 {
		return fmt.Errorf("account query HTTP %d: %s", resp.StatusCode, string(body))
	}
...
func (w *TaskWorker) publishTaskStatus(activeJob *ActiveJob, message, errorMsg string) {
	...
	if activeJob.BillingSession != nil {
		update.ActualCostCLAW = activeJob.BillingSession.CurrentCost
	}
...
// collectAMDMetrics collects AMD GPU metrics using rocm-smi
func (w *TaskWorker) collectAMDMetrics() ([]GPUMetrics, error) {
	...
	cmd := exec.Command("rocm-smi", "--showid", "--showtemp", "--showuse", "--showmemuse", "--showpower", "--showclocks", "--csv")
	output, err := cmd.Output()
	if err != nil {
	...
	lines := strings.Split(strings.TrimSpace(string(output)), "\n")
	if len(lines) < 2 {
		return nil, fmt.Errorf("rocm-smi returned no data")
	}
	...
	for i, line := range lines[1:] {
		...
		temperature, _ := strconv.ParseUint(strings.TrimSpace(parts[1]), 10, 8)
		utilizationGPU, _ := strconv.ParseUint(strings.TrimSpace(strings.TrimSuffix(strings.TrimSpace(parts[2]), "%")), 10, 8)
		memUsedMB, _ := strconv.ParseUint(strings.TrimSpace(parts[3]), 10, 64)
		powerDraw, _ := strconv.ParseFloat(strings.TrimSpace(parts[4]), 64)
		clockCore, _ := strconv.ParseUint(strings.TrimSpace(parts[5]), 10, 32)

		metric := GPUMetrics{
			Index:          i,
			UUID:           fmt.Sprintf("amd-%d", i),
...
// collectAppleMetrics collects Apple Silicon GPU metrics using system_profiler
func (w *TaskWorker) collectAppleMetrics() ([]GPUMetrics, error) {
	...
	output, err := cmd.Output()
	if err != nil {
		return nil, fmt.Errorf("system_profiler execution failed: %w", err)
	}
	...
	var metrics []GPUMetrics
	displays, ok := spData["SPDisplaysDataType"].([]interface{})
	if !ok || len(displays) == 0 {
		return nil, fmt.Errorf("no GPU data from system_profiler")
	}

	for i, display := range displays {
		gpuData, ok := display.(map[string]interface{})
		if !ok {
			continue
		}

		name := "Apple Silicon GPU"
		if n, ok := gpuData["sppci_model"].(string); ok {
			name = n
		}

		var vramTotal uint64
		if vram, ok := gpuData["spdisplays_vram_shared"].(string); ok {
			// Parse e.g. "16 GB" → 16384 MB
			vram = strings.TrimSpace(strings.TrimSuffix(vram, "GB"))
			if gb, err := strconv.ParseUint(strings.TrimSpace(vram), 10, 64); err == nil {
				vramTotal = gb * 1024
			}
		}
