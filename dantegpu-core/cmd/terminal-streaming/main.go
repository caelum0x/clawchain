...
func (ts *TerminalStreamer) RunComprehensiveTest() {
	...
	// Step 7: Check Billing Service health and CLAW token status
	currentStep++
	ts.LogInfoWithProgress("Health Checks", "curl", fmt.Sprintf("[%d/%d] Checking Billing Service health and CLAW token processing...", currentStep, totalSteps), float32(currentStep)/float32(totalSteps)*100)
	...
	currentStep++
	ts.LogInfoWithProgress("Rental Binary", "auth", fmt.Sprintf("[%d/%d] Testing rental binary - authentication, CLAW payments, and job submission...", currentStep, totalSteps), float32(currentStep)/float32(totalSteps)*100)
	go func() {
		...
		ts.LogInfo("Rental Binary", "auth", "Expected: CLAW wallet initialization and balance check")
		ts.LogInfo("Rental Binary", "auth", "Expected: ClawChain blockchain integration")
		...
		go func() {
			...
			for scanner.Scan() {
				...
				} else if strings.Contains(line, "CLAW") || strings.Contains(line, "token") {
					ts.LogSuccess("Rental Binary", "payment", fmt.Sprintf("CLAW Payment: %s", line))
				...
				} else if strings.Contains(line, "ClawChain") || strings.Contains(line, "blockchain") {
	...
	// Step 14: Check CLAW token pricing and marketplace status
	currentStep++
	ts.LogInfoWithProgress("Billing Service", "curl", fmt.Sprintf("[%d/%d] Checking CLAW token pricing and marketplace rates...", currentStep, totalSteps), float32(currentStep)/float32(totalSteps)*100)
	...
	// Step 18: Check blockchain integration and ClawChain connection
	currentStep++
	ts.LogInfoWithProgress("Blockchain", "clawchain", fmt.Sprintf("[%d/%d] Verifying ClawChain blockchain integration and CLAW token status...", currentStep, totalSteps), float32(currentStep)/float32(totalSteps)*100)
	ts.ExecuteCommand("Blockchain", "curl", "-s", "http://localhost:1317/cosmos/base/tendermint/v1beta1/node_info")
	...
	ts.LogInfo("Job", "scheduler", "Expected CLAW cost calculation: Based on VRAM usage and duration")
	ts.LogInfo("Job", "scheduler", "Expected payment flow: CLAW token deduction from wallet")
	...
	ts.LogSuccess("DanteGPU Testing", "summary", "Payment System: CLAW token integration verified")
	ts.LogSuccess("DanteGPU Testing", "summary", "Blockchain: ClawChain integration operational")
