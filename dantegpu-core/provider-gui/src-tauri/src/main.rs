...
async fn get_local_jobs(app_handle: tauri::AppHandle) -> Result<Vec<LocalJob>, String> {
    emit_log_entry(&app_handle, "status", "Fetching local jobs from daemon...".to_string());

    let daemon_path = find_daemon_binary(&app_handle)?;
    let output = std::process::Command::new(&daemon_path)
        .arg("--get-local-jobs-json")
        .output()
        .map_err(|e| format!("Failed to run daemon binary: {}", e))?;

    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        emit_log_entry(&app_handle, "warn", format!("Daemon --get-local-jobs-json failed: {}", stderr));
        return Ok(vec![]);
    }

    let stdout = String::from_utf8_lossy(&output.stdout);
    let jobs: Vec<LocalJob> = serde_json::from_str(&stdout).unwrap_or_else(|e| {
        emit_log_entry(&app_handle, "warn", format!("Failed to parse jobs JSON: {}", e));
        vec![]
    });
