...
func TestUnshieldHandlerE2E(t *testing.T) {
	...
	// The shield handler uses blinding = commitCount + 1.
	// Before the shield, commitCount was 0, so blinding = 1.
...
func TestTreeStatsAfterShields(t *testing.T) {
	...
	// Shield twice.
	_, err = msgServer.Shield(f.ctx, &types.MsgShield{Creator: creatorAddr, Amount: 100, Coins: "stake"})
	require.NoError(t, err)
	_, err = msgServer.Shield(f.ctx, &types.MsgShield{Creator: creatorAddr, Amount: 200, Coins: "stake"})
	require.NoError(t, err)
...
func TestBatchPrivateTransferSuccess(t *testing.T) {
	...
	for i := uint64(0); i < 4; i++ {
		...
		require.NoError(t, err)
		// Shield handler uses blinding = commitCount_before_increment + 1
		// For shield i: commitCount was i when Peek was called, so blinding = i + 1
		utxos[i] = utxoInfo{
			amount:     new(big.Int).SetUint64(uint64(100 * (i + 1))),
			blinding:   new(big.Int).SetUint64(i + 1),
			commitment: commit,
			leafIdx:    idx,
		}
	}
