...
import { ClawDEXConsumer, ClawDEXProvider } from "modules/common";
import { ClawWebappProvider } from "context/ClawWebappContext";
...
const CLAWDEX_V2_LINK = "https://app.clawchain.io/";

const Layout: FC = ({ children }) => {
  const [isMobile] = useMediaQuery(`(max-width: ${MOBILE_MAX_WIDTH})`);
  const isInitializing = false; // Keplr doesn't have an initializing state
  const spinner = (
    <Center h="full">
  ...
  const errorLoadingData = (
    <Center h="full">
      <Box color="red.500" px={4} textAlign="center">
        Error loading data. The ClawChain public hive infrastructure that
        ClawDEX relies on is currently offline.
      </Box>
    </Center>
  );
        ...
        <ClawWebappProvider>
          <ClawDEXProvider>
            {!isMobile && (
              ...
              >
                <Text textStyle="h2">
                  You are on ClawDEX Classic. For ClawDEX V2 go to{" "}
                </Text>
                <chakra.a
                  href={CLAWDEX_V2_LINK}
