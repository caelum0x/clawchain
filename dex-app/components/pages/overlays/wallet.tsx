...
const Wallet: FC<Props> = ({ onClose, type = "popover" }) => {
  ...
  const { disconnect } = useKeplrWallet();
  ...
  const formattedBalance = isLow ? "< 0.01" : fromClawAmount(balance);
  ...
  const copyAddress = () => {
    ...
    toast({
      ...
      description: "Your ClawChain address is now in your clipboard",
      status: "info",
      duration: 2000,
      isClosable: false,
    });
    ...
    <>
      ...
      >
        ...
        <Flex mt={6} justify="space-between">
          ...
          <Link isExternal href={finder(walletAddress)}>
            <HStack>
