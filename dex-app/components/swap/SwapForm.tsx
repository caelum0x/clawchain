...
const SwapForm: FC<Props> = ({ defaultToken1, defaultToken2 }) => {
  ...
  const chainID = process.env.NEXT_PUBLIC_CHAIN_ID || "clawchain-1";
  ...
  const onSubmit = useCallback(() => {
    ...
    if (taxEnabled && fee) {
      // For CosmJS StdFee, combine gas fee + tax into the amount array
      const taxAmount = calcMinimumTaxAmount(num(amount1).times(num(10).pow(6)), {
        taxRate,
        taxCap,
      });
      const existingAmounts = fee.amount || [];
      const taxCoin = { denom: token1, amount: String(taxAmount) };
      submitFee = {
        gas: fee.gas,
        amount: [...existingAmounts, taxCoin],
      };
    } else {
      submitFee = fee;
    }
