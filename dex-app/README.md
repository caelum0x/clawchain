...
The ClawDEX UI is a web frontend used for interacting with the ClawDEX Smart Contracts on ClawChain.
...
ClawDEX UI provides a notification system that uses Toast-style notifications to provide feedback to the user.

Notifications can be added via the `addNotification` function returned by the `useClawDEX` hook.
...
ClawDEX UI provides two hooks designed to simplify transaction posting:

- `useTx` -- Thin wrapper around the wallet provider's `post` function. Can be configured with `onPosting`, `onBroadcasting`, and `onError` lifecycle hooks. Returns a `submit` function that accepts Messages and a Fee. Hooks into ClawDEX's notification and transaction tracking systems to provide feedback to the user via Toast notifications.
...
When adding new transactions to the ClawDEX UI, the following tasks must be completed:
