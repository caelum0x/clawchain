...
export const useLockdropRewards = () => {
  ...
  return useMemo(() => {
    ...
    userInfo.lockup_infos.forEach((info: any) => {
      if (num(info.claimable_generator_claw_debt).gt(0)) {
        list.push({
          token: clawToken,
          amount: num(info.claimable_generator_claw_debt)
            .div(10 ** getDecimals(clawToken))
            .dp(6)
            .toNumber(),
          lp: info.clawdex_lp_token,
          claimLp: info.clawswap_lp_token,
      ...
      if (
        num(info.claimable_generator_claw_debt).gt(0) ||
        num(info.claimable_generator_proxy_debt).gt(0)
      ) {
        data.push({
          duration: info.duration,
          lp: info.clawswap_lp_token,
        });
      }
    });
