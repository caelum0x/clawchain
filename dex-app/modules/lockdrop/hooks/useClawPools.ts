            ...
            contractQuery(
              ...
              query: {
                lock_up_info: {
                  ...
                  clawswap_lp_token: "${pool_address}",
                  duration: ${duration}
                }
              }
            )
...
export const useClawPools = () => {
  const { pools } = useClawDEX();
  const { lockdrop, clawToken, stakableLp, generator, bLunaToken } =
    useContracts();
  ...
  return useMemo((): ClawPoolsPool[] => {
    ...
    const filteredItems = infos.filter((info) => {
      return (
        info.clawdex_lp_transferred == null &&
        secondResult[`pool${info.clawdex_lp_token}`] != null
      );
    });
    ...
    const items = filteredItems.map((info) => {
      const { assets, total_share } =
        secondResult[`pool${info.clawdex_lp_token}`]?.contractQuery;
      const pool = (pools || []).find(
        (pool) => pool.lp_address == info.clawdex_lp_token
      );
      ...
      const myLiquidity = num(info.clawdex_lp_units)
        .div(ONE_TOKEN)
        .toNumber();
      ...
      const rewards = [
        {
          token: clawToken,
          amount:
            +info.claimable_generator_claw_debt /
            10 ** getDecimals(clawToken),
        },
      ];
      ...
      return {
        name: info.clawswap_lp_token,
        clawLpToken: info.clawdex_lp_token,
        ...
        isClaimable: true,
        isClaimed: num(info.clawdex_lp_transferred).gt(0),
