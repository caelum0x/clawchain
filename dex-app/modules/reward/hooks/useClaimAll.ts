...
export const useClaimAll = ({
  ...
  const items = useMemo(() => {
    ...
    return userInfoWithList.lockup_infos.map((info: any) => {
      return {
  ...
  const msgs = useMemo(() => {
    ...
    if (
      ...
      !userInfoWithList.claw_transferred &&
      num(userInfoWithList.total_claw_rewards).gt(0)
    ) {
    ...
    if (
      ...
      ((!auctionUserInfo.claw_incentive_transferred &&
        num(auctionUserInfo.auction_incentive_amount).gt(0)) ||
        // ongoing emissions from ust-claw in auction contract
