...
export const composeTwitterLink = (title: string = "", id: string) => {
  return (
    `https://twitter.com/intent/tweet?text=New ClawDEX proposal 🚀%0A%0A` +
    // @ts-ignore
    `${title}%0A%0A&url=https://app.clawchain.io/governance/proposal/${id}`
  );
};

export const composeClawRatioDisplay = (
...
export const composeProtocolRatioDisplay = (
  stakedClawBalance?: string,
  xClawSupply?: string,
  clawCircSupply?: number,
  stakingRatio?: number
): string => {
  if (
    stakedClawBalance === undefined ||
    xClawSupply === undefined ||
    clawCircSupply === undefined ||
    stakingRatio === undefined
  ) {
    return `-`;
  }

  if (stakedClawBalance === "0") {
    return `0%`;
  }
  ...
  return stakedClawBalance && clawCircSupply
    ? `${handleTinyAmount(
        Number(stakedClawBalance) / (clawCircSupply * 10 ** 4)
      )}%`
    : `-`;
};
