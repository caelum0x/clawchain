...
export const handleBigAndTinyAmount = (
  ...
  if (includeZero && num(value).eq(0)) {
    return `< ${numberPrefix}0.01${includeDollarSign ? " CLAW" : ""}`;
  }
  ...
  if (num(value).gt(1000000)) {
    return `${numberPrefix}${numeral(value)
      ...
      .toUpperCase()}${includeDollarSign ? " CLAW" : ""}`;
  }

  return `${numberPrefix}${numeral(value).format(format)}${
    includeDollarSign ? " CLAW " : ""
  }`;
};
...
export const handleDollarTinyAmount = (
  ...
  return handleTinyAmount(value, format, includeZero, " CLAW");
};
...
export const getEventsByType = (txInfo: any, index: number = 0): any => {
  if (!txInfo) {
    return null;
  }
