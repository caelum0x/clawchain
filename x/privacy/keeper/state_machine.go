...
// Each nullifier is stored in state immediately after passing the Has() check to prevent
// same-block double-spend attacks where two transactions reference the same nullifier.
func (k Keeper) ConsumeNullifiers(ctx context.Context, nullifierHexes []string) error {
	...
	for _, raw := range nullifierHexes {
		...
		// Store the nullifier immediately after the Has() check passes.
		// This prevents same-block double-spend: if another tx in the same block
		// tries to use this nullifier, it will see it already exists.
