...
func TestListSkillInfersCategoryFromKeywords(t *testing.T) {
	f := initFixture(t)
	msgServer := keeper.NewMsgServerImpl(f.keeper)

	resp, err := msgServer.ListSkill(f.ctx, &types.MsgListSkill{
		Creator:     validAddress(),
		Name:        "GPU Inference Optimizer",
		Description: "CUDA model acceleration",
		Price:       "500",
		Denom:       "uclaw",
	})
	require.NoError(t, err)
	...
	skill, err := f.keeper.Skills.Get(f.ctx, resp.SkillId)
	require.NoError(t, err)
	require.Equal(t, "ai-ml", skill.Category)
}

func TestListSkillCategoryFallsBackToInputToken(t *testing.T) {
	f := initFixture(t)
	msgServer := keeper.NewMsgServerImpl(f.keeper)
	...
	resp, err := msgServer.ListSkill(f.ctx, &types.MsgListSkill{
		...
		Description: "Niche budgeting helper",
		Price:       "500",
		Denom:       "uclaw",
	})
	require.NoError(t, err)
	...
	skill, err := f.keeper.Skills.Get(f.ctx, resp.SkillId)
	require.NoError(t, err)
	require.Equal(t, "finance", skill.Category)
}
