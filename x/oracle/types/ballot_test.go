...
func TestToCrossRate(t *testing.T) {
	...
	for _, data := range data {
		...
		if !data.base.IsZero() {
			pbBase = append(pbBase, types.NewVoteForTally(data.base, "uatom", valAddr, 100))
		}

		pbQuote = append(pbQuote, types.NewVoteForTally(data.quote, "uatom", valAddr, 100))

		if !data.base.IsZero() && !data.quote.IsZero() {
			cb = append(cb, types.NewVoteForTally(data.base.Quo(data.quote), "uatom", valAddr, 100))
		} else {
			cb = append(cb, types.NewVoteForTally(sdkmath.LegacyZeroDec(), "uatom", valAddr, 0))
		}
	}
