...
The Oracle module provides ClawChain with up-to-date and accurate price feeds of exchange rates of CLAW against various denominations so that on-chain services (DEX, marketplace, agent economy) have fair pricing data.

As price information is extrinsic to the blockchain, the ClawChain network relies on validators to periodically vote on current exchange rates, with the protocol tallying up the results once per `VotePeriod` and updating the on-chain exchange rate as the weighted median of the ballot.
