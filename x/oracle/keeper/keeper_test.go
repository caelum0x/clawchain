...
func TestExchangeRate(t *testing.T) {
	...
	// Set & get rates
	input.OracleKeeper.SetLunaExchangeRate(input.Ctx, "ueth", cnyExchangeRate)
	rate, err := input.OracleKeeper.GetLunaExchangeRate(input.Ctx, "ueth")
	require.NoError(t, err)
	require.Equal(t, cnyExchangeRate, rate)

	input.OracleKeeper.SetLunaExchangeRate(input.Ctx, "uusdt", gbpExchangeRate)
	rate, err = input.OracleKeeper.GetLunaExchangeRate(input.Ctx, "uusdt")
	require.NoError(t, err)
	require.Equal(t, gbpExchangeRate, rate)

	input.OracleKeeper.SetLunaExchangeRate(input.Ctx, "uatom", krwExchangeRate)
	rate, err = input.OracleKeeper.GetLunaExchangeRate(input.Ctx, "uatom")
	require.NoError(t, err)
	...
	input.OracleKeeper.DeleteLunaExchangeRate(input.Ctx, "uatom")
...
func TestRewardPool(t *testing.T) {
	...
	fees := sdk.NewCoins(sdk.NewCoin("uusd", sdkmath.NewInt(1000)))
	...
	KFees := input.OracleKeeper.GetRewardPool(input.Ctx, "uusd")
...
func TestParams(t *testing.T) {
	...
	whitelist := types.DenomList{
		{Name: "uusd", TobinTax: types.DefaultTobinTax},
		{Name: "uatom", TobinTax: types.DefaultTobinTax},
	}
...
func TestTobinTaxGetSet(t *testing.T) {
	...
	// Clear default whitelist tobin taxes before setting test values
	input.OracleKeeper.ClearTobinTaxes(input.Ctx)

	tobinTaxes := map[string]sdkmath.LegacyDec{
		"uusd":  sdkmath.LegacyNewDec(1),
		"uatom": sdkmath.LegacyNewDecWithPrec(123, 3),
		"ubtc":  sdkmath.LegacyNewDecWithPrec(1423, 4),
		"ueth":  sdkmath.LegacyNewDecWithPrec(1, 3),
	}
