...
func TestQueryExchangeRate(t *testing.T) {
	...
	input.OracleKeeper.SetLunaExchangeRate(input.Ctx, "uusd", rate)
...
func TestQueryExchangeRates(t *testing.T) {
	...
	rate := sdkmath.LegacyNewDec(1700)
	input.OracleKeeper.SetLunaExchangeRate(input.Ctx, "uusd", rate)
	input.OracleKeeper.SetLunaExchangeRate(input.Ctx, "uatom", rate)

	res, err := querier.ExchangeRates(ctx, &types.QueryExchangeRatesRequest{})
	require.NoError(t, err)

	require.Equal(t, sdk.DecCoins{
		sdk.NewDecCoinFromDec("uatom", rate),
		sdk.NewDecCoinFromDec("uusd", rate),
	}, res.ExchangeRates)
}
...
func TestQueryActives(t *testing.T) {
	...
	input.OracleKeeper.SetLunaExchangeRate(input.Ctx, "uusd", rate)
	input.OracleKeeper.SetLunaExchangeRate(input.Ctx, "uatom", rate)
	input.OracleKeeper.SetLunaExchangeRate(input.Ctx, "uusdt", rate)
...
func TestQueryTobinTax(t *testing.T) {
	...
	denom := types.Denom{Name: "uatom", TobinTax: sdkmath.LegacyOneDec()}
