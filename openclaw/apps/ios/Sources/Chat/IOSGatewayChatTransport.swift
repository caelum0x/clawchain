...
struct IOSGatewayChatTransport: OpenClawChatTransport, Sendable {
    ...
    func requestChainStatus() async -> ChainStatusResponse? {
        do {
            let res = try await self.gateway.request(method: "chain.status", paramsJSON: nil, timeoutSeconds: 10)
            return try JSONDecoder().decode(ChainStatusResponse.self, from: res)
        } catch {
            Self.logger.error("chain.status failed \(error.localizedDescription, privacy: .public)")
            return nil
        }
    }
