...
import ai.openclaw.android.chain.ChainStatusViewModel
import androidx.compose.material.icons.filled.Link
import androidx.lifecycle.viewmodel.compose.viewModel

@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun RootScreen(viewModel: MainViewModel, chainViewModel: ChainStatusViewModel = viewModel()) {
  ...
  Popup(alignment = Alignment.TopEnd, properties = PopupProperties(focusable = false)) {
    Column(
      ...
      OverlayIconButton(
        onClick = { sheet = Sheet.Chain },
        icon = { Icon(Icons.Default.Link, contentDescription = "Chain") },
      )
  ...
  if (currentSheet != null) {
    ModalBottomSheet(
      ...
      when (currentSheet) {
        ...
        Sheet.Chain -> Column {
          ChainStatusCard(state = chainState, modifier = Modifier.padding(16.dp))
          ChainActionsSheet(onSendMessage = { msg -> viewModel.sendChatMessage(msg); sheet = null })
        }
      }
    }
  }
