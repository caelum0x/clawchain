...
describe("config schema", () => {
  ...
  it("includes blockchain heartbeat and extension runtime fields", () => {
    const res = buildConfigSchema();
    const schema = res.schema as { properties?: Record<string, unknown> };
    const blockchain = schema.properties?.blockchain as
      | { properties?: Record<string, unknown> }
      | undefined;
    expect(blockchain?.properties?.heartbeat).toBeTruthy();
    expect(blockchain?.properties?.messagingEndpoint).toBeTruthy();
    expect(blockchain?.properties?.faucet).toBeTruthy();
    expect(blockchain?.properties?.peers).toBeTruthy();
  });
});
