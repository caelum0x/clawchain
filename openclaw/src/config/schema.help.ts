...
export const FIELD_HELP: Record<string, string> = {
  ...
  "blockchain.messagingEndpoint":
    "Public endpoint this agent advertises for on-chain messaging (used during agent registration).",
  "blockchain.heartbeat.enabled":
    "Enable periodic on-chain heartbeat actions from the ClawChain extension (default: true).",
  "blockchain.heartbeat.intervalSeconds":
    "Heartbeat interval in seconds (minimum 15, default 60).",
  "blockchain.heartbeat.includeNodeStatus":
    "Attach latest observed node status (height/mode) to heartbeat payloads (default: true).",
  "blockchain.faucet.enabled":
    "Run the local faucet server for development/testnet token drip (default: false).",
  "blockchain.faucet.port": "HTTP port for the local faucet server (default: 8888).",
  "blockchain.faucet.dripAmount":
    "Base-denom amount dispensed per faucet request (default: 10000000 = 10 CLAW).",
  "blockchain.faucet.url":
    "External faucet URL override when using a remote faucet instead of local server.",
  "blockchain.peers.seeds": "Comma-separated seed nodes for CometBFT peer discovery.",
  "blockchain.peers.persistentPeers":
    "Comma-separated persistent peers for long-lived node connectivity.",
  "gateway.auth.token":
