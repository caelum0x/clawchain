...
import { resolveUniqueSlug } from "./device-slug.js";
...
export async function approveDevicePairing(
  ...
  return await withLock(async () => {
    ...
    const existingSlugs = new Set(
      Object.values(state.pairedByDeviceId)
        .map((d) => d.slug)
        .filter((s): s is string => Boolean(s)),
    );
    const slug = existing?.slug ?? resolveUniqueSlug(pending.deviceId, existingSlugs);
