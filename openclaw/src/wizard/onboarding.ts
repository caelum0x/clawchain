...
export async function runOnboardingWizard(
  ...
  // Setup ClawChain blockchain integration (Install → Run → Earn)
  const { setupBlockchain } = await import("./onboarding.blockchain.js");
  const blockchainResult = await setupBlockchain(nextConfig, { flow, prompter, runtime });
  nextConfig = blockchainResult.nextConfig;
