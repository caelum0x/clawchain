    ...
    try {
      ...
      if (!client) {
        ...
        if (!device) {
          ...
          // Enforce mandatory device auth for remote connections when configured
          const requireDeviceAuthCfg = configSnapshot.gateway?.auth?.requireDeviceAuth === true;
          if (requireDeviceAuthCfg && !isLocalClient && !isWebchat) {
            markHandshakeFailure("device-required-by-policy");
            sendHandshakeErrorResponse(
              ErrorCodes.NOT_PAIRED,
              "device identity required for remote connections",
            );
            close(1008, "device identity required for remote connections");
            return;
          }
