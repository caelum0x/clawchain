...
export type GatewayBroadcastToOperatorsFn = (
  event: string,
  payload: unknown,
  opts?: GatewayBroadcastOpts,
) => void;

/** Events that nodes should always receive regardless of operator-only routing. */
const NODE_SAFE_EVENTS = new Set([
  "node.invoke.request",
  "tick",
  "shutdown",
  "voicewake.changed",
...
export function createGatewayBroadcaster(params: { clients: Set<GatewayWsClient> }) {
  ...
  const broadcastToOperators: GatewayBroadcastToOperatorsFn = (event, payload, opts) => {
    if (params.clients.size === 0) {
      return;
    }
    const eventSeq = ++seq;
    ...
    for (const c of params.clients) {
      const role = c.connect.role ?? "operator";
      if (role === "node") {
        continue;
      }
      if (!hasEventScope(c, event)) {
  ...
  return { broadcast, broadcastToConnIds, broadcastToOperators };
}
