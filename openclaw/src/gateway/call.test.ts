...
describe("callGatewayChain wrappers", () => {
  beforeEach(() => {
    loadConfig.mockReset();
    resolveGatewayPort.mockReset();
    pickPrimaryTailnetIPv4.mockReset();
  ...
  it("maps typed chain call wrappers to the expected gateway methods", async () => {
    await callGatewayChain.status();
    expect(lastRequest).toEqual({ method: "chain.status", params: {}, opts: { expectFinal: undefined } });

    await callGatewayChain.runtimeStatus();
    expect(lastRequest).toEqual({ method: "runtime.status", params: {}, opts: { expectFinal: undefined } });

    await callGatewayChain.agents.list({ limit: 10 });
    ...
    await callGatewayChain.agents.info({ address: "claw1agent" });
    expect(lastRequest).toEqual({
      method: "chain.agents.info",
      params: { address: "claw1agent" },
      opts: { expectFinal: undefined },
    });

    await callGatewayChain.agents.tasks({ role: "delegator" });
    expect(lastRequest).toEqual({
      method: "chain.agents.tasks",
      params: { role: "delegator" },
      opts: { expectFinal: undefined },
    });

    await callGatewayChain.agents.delegate({ assignee: "claw1worker", description: "Run task" });
    expect(lastRequest).toEqual({
      method: "chain.agents.delegate",
      params: { assignee: "claw1worker", description: "Run task" },
      opts: { expectFinal: undefined },
    });

    await callGatewayChain.agents.reputation({ address: "claw1agent" });
    expect(lastRequest).toEqual({
    ...
    await callGatewayChain.wallet.balance({ denom: "uclaw" });
    expect(lastRequest).toEqual({
    ...
    await callGatewayChain.wallet.transfer({ recipient: "claw1dest", amount: "9" });
    expect(lastRequest).toEqual({
      method: "chain.wallet.transfer",
      params: { recipient: "claw1dest", amount: "9" },
      opts: { expectFinal: undefined },
    });

    await callGatewayChain.wallet.stakingDelegations();
    expect(lastRequest).toEqual({
    ...
    await callGatewayChain.wallet.stakingRewards();
    expect(lastRequest).toEqual({
    ...
    await callGatewayChain.wallet.history({ limit: 5 });
    expect(lastRequest).toEqual({
