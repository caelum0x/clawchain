...
const entries: SubCliEntry[] = [
  {
    name: "provider",
    description: "Activate ClawChain provider mode from OpenClaw",
    hasSubcommands: true,
    register: async (program) => {
      const mod = await import("../provider-cli.js");
      mod.registerProviderCli(program);
    },
  },
  {
    name: "up",
    description: "Start bot + chain runtime with one command",
    hasSubcommands: false,
    register: async (program) => {
      const mod = await import("../up-cli.js");
      mod.registerUpCli(program);
    },
  },
  {
    name: "agent-flow",
    description: "Run core on-chain agent lifecycle (direct SDK path)",
    hasSubcommands: false,
    register: async (program) => {
      const mod = await import("../agent-flow-cli.js");
      mod.registerAgentFlowCli(program);
    },
  },
  {
    name: "product-flow",
    description: "Run end-to-end product lifecycle via delegated clawd flow",
    hasSubcommands: false,
    register: async (program) => {
      const mod = await import("../product-flow-cli.js");
      mod.registerProductFlowCli(program);
    },
  },
  {
    name: "acp",
