...
import { doctorRuntimeCommand } from "../../commands/doctor-runtime.js";
...
export function registerMaintenanceCommands(program: Command) {
  ...
  doctor
    .command("runtime")
    .description("Preflight bot+node runtime prerequisites (OpenClaw + clawd + clawchain)")
    .option("--json", "Output machine-readable JSON", false)
    .option("--repair", "Auto-fix clawd readiness blockers when possible (build/config/mnemonic/agentAddress/manifest rejoin)", false)
    .action(async (opts) => {
      await runCommandWithRuntime(defaultRuntime, async () => {
        const result = await doctorRuntimeCommand(defaultRuntime, {
