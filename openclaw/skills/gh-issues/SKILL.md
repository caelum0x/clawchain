...
IMPORTANT — No `gh` CLI dependency. This skill uses curl + the GitHub REST API exclusively. The GH_TOKEN env var is already injected by Claw. Pass it as a Bearer token in all API calls:
...
- If curl returns an HTTP 401 or 403 → stop and tell the user:
  > "GitHub authentication failed. Please check your apiKey in the Claw dashboard or in ~/.openclaw/openclaw.json under skills.entries.gh-issues."
