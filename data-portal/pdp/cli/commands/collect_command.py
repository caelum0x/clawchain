...
def collect_command(
    ...
    elif network == 'clawchain':
        from pdp.datasets import clawchain as claw_ds
        from pdp.datasets.clawchain.client import ClawchainClient

        # Build a ClawChain client using the provided RPC url or defaults
        client_kwargs = {}
        if rpc is not None:
            client_kwargs['rpc_url'] = rpc
        claw_client = ClawchainClient(**client_kwargs)

        collector = {
            'blocks': claw_ds.collect_blocks,
            'transactions': claw_ds.collect_transactions,
            'agents': claw_ds.collect_agents,
            'prices': claw_ds.collect_prices,
            'privacy': claw_ds.collect_privacy,
        }.get(datatype)

        if collector is None:
            raise Exception('unknown clawchain datatype: ' + str(datatype))

        # blocks and transactions accept height ranges; others don't
        if datatype in ('blocks', 'transactions'):
            collector(
