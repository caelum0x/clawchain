...
const CLAWCHAIN_API_URL = import.meta.env.VITE_CLAWCHAIN_API_URL || 'https://registry.clawchain.io';
const GITHUB_API_URL =
  import.meta.env.VITE_GITHUB_API_URL || 'https://api.github.com/repos/cosmos/chain-registry/contents';
const IBC_API_URL = IBC_USE_GITHUB_API ? GITHUB_API_URL : CLAWCHAIN_API_URL;
  ...
  actions: {
    load() {
      ...
      const client = new ChainRegistryClient({
        ...
        baseUrl: IBC_USE_GITHUB_API ? undefined : new URL(`${prefix}`, CLAWCHAIN_API_URL + '/').toString(),
      });
    ...
    async fetchIBCUrls(): Promise<any[]> {
      ...
      // If using CLAWCHAIN_API_URL, add thedownload URLs
      if (IBC_API_URL == CLAWCHAIN_API_URL) {
        return entries.map((entry: any) => {
          entry.download_url = new URL(`${prefix}_IBC/${entry.name}`, CLAWCHAIN_API_URL + '/').toString();
          return entry;
        });
      }
      return entries;
