...
export function convertFromLocal(lc: LocalChainConfig): ChainConfig {
  ...
  conf.logo = lc.logo.startsWith('http') ? lc.logo : `https://clawchain.io${lc.logo}`;
...
function pathConvert(path: string | undefined) {
  if (path) {
    path = path.replace('https://raw.githubusercontent.com/cosmos/chain-registry/master', 'https://registry.clawchain.io');
  }
  return path || '';
}
...
export const useDashboard = defineStore('dashboard', {
  state: () => {
    const favMap = JSON.parse(localStorage.getItem('favoriteMap') || '{"clawchain":true, "cosmos":true}');
    return {
