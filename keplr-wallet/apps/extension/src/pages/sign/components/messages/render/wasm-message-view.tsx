...
export const WasmMessageView: FunctionComponent<{
  ...
  useEffect(() => {
    ...
    // So, try to decrypt.
    // But, if this msg is not encrypted via Claw Wallet, Claw Wallet cannot decrypt it.
    // TODO: Handle the error case. If an error occurs, rather than rejecting the signing, it informs the user that Claw Wallet cannot decrypt it and allows the user to choose.
    if (isSecretWasm && typeof msg === "string") {
      (async () => {
        try {
          ...
          setWarningMsg(
            "Failed to decrypt Secret message. This may be due to Claw Wallet's encrypt/decrypt seed not matching the transaction seed."
          );
        }
      })();
    }
